...
const Sidebar: React.FC<SidebarProps> = ({ isMobileOpen = false, onMobileClose }) => {
  ...
  const [isCentroOpsOpen, setIsCentroOpsOpen] = useState(false);
  ...
  const sidebarContent = (
    <>
      ...
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
        ...
        <div className="pt-2">
          <div
            className={`flex items-center justify-between px-3 py-2 rounded-md cursor-pointer transition-colors ${isGroupActive('/ops') && !isGroupActive('/ops/operator') && !isGroupActive('/ops/orchestrator') ? 'bg-white/5 text-white' : 'text-text-secondary hover:text-white'}`}
          ...
          {isOpsOpen && (
            <div className="mt-1 space-y-1 pb-2">
              <div className="space-y-1">
                ...
                <div className={subLinkClass('/ops/picks/analysis')} onClick={() => handleNavigate('/ops/picks/analysis')}>An├ílisis</div>
              ...
              <div className="space-y-1">
                ...
                <div className={subLinkClass('/ops/valuebets/analysis')} onClick={() => handleNavigate('/ops/valuebets/analysis')}>An├ílisis</div>
        ...
        {/* --- CENTRO OPERATIVO GROUP --- */}
        <div className="pt-2">
          <div
            className={`flex items-center justify-between px-3 py-2 rounded-md cursor-pointer transition-colors ${isGroupActive('/centro-operativo') || isGroupActive('/ops/operator') || isGroupActive('/ops/orchestrator') ? 'bg-white/5 text-white' : 'text-text-secondary hover:text-white'}`}
            onClick={() => setIsCentroOpsOpen(!isCentroOpsOpen)}
          >
            <div className="flex items-center gap-3">
              <span className="material-symbols-outlined">psychology</span>
              <span className="text-sm font-semibold">Centro Operativo</span>
            </div>
            <span className={`material-symbols-outlined text-sm transition-transform ${isCentroOpsOpen ? 'rotate-180' : ''}`}>expand_more</span>
          </div>

          {isCentroOpsOpen && (
            <div className="mt-1 space-y-1 pb-2">
              <div className={subLinkClass('/centro-operativo')} onClick={() => handleNavigate('/centro-operativo')}>Dashboard</div>
              <div className={subLinkClass('/ops/operator')} onClick={() => handleNavigate('/ops/operator')}>Terminal Operador</div>
              <div className={subLinkClass('/ops/orchestrator')} onClick={() => handleNavigate('/ops/orchestrator')}>Orchestrator</div>
            </div>
          )}
        </div>

        {/* --- RED OPERATIVA (INDEPENDENT) --- */}

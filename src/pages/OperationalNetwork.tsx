...
interface AgencyApiResponse {
   ...
   rake_percentage_display: string;
   movements_current_month: number;
   ggr_current_month: number;
   estimated_commission: number;
   active_profiles_count: number;
   missing_profiles_count: number;
   alerts: string[];
}
...
const OperationalNetwork: React.FC = () => {
      ...
      const fetchAgencies = async () => {
         try {
            ...
            const response = await authService.authenticatedFetch(
               `${BASE_URL}/api/administration/agencies/`
            );
            ...
            const data: AgencyApiResponse[] = await response.json();

            // Mapear los datos de la API al formato interno
            const mappedAgencies: Agency[] = data.map((apiAgency) => ({
               id: apiAgency.id_display,
               owner: apiAgency.owner,
               bookmaker: apiAgency.house,
               username: '', // No viene en la API
               password: '', // No viene en la API
               backofficeUrl: '', // No viene en la API
               rakePercent: parseFloat(apiAgency.rake_percentage),
               movements: apiAgency.movements_current_month,
               houseGGR: apiAgency.ggr_current_month,
               status: apiAgency.status === 'ACTIVO' ? 'Active' : 'Blocked',
               lastSyncAt: new Date().toISOString(),
               notes: [],
               profiles: Array.from({ length: apiAgency.active_profiles_count }).map((_, i) => ({
                  id: `P-${apiAgency.id_display}-${i + 1}`,
                  status: 'Activo',
                  balance: 0,
                  lastActivity: 'Hace 2 horas'
               })),
               minRequiredProfiles: apiAgency.active_profiles_count + apiAgency.missing_profiles_count,
               balanceBackoffice: 0
            }));
   ...
   return (
      <div className="flex-1 flex flex-col h-full bg-[#0a0b0e] text-slate-300 relative overflow-hidden">
         ...
         <div className="px-6 py-4 border-b border-border-dark bg-surface-dark/95 backdrop-blur-md sticky top-0 z-40 flex flex-col gap-4">
            <div className="flex justify-between items-center">
               <div className="flex items-center gap-3">
                  <div className="p-2.5 bg-primary/20 rounded-xl text-primary border border-primary/20"><LayoutDashboard size={24} /></div>
                  <div>
                     <h1 className="text-xl font-black text-white tracking-tight">RED OPERATIVA</h1>
                     <p className="text-[10px] text-text-secondary font-bold uppercase tracking-[0.2em]">Network Command Center • Gestión de Agencias</p>
                  </div>
               </div>

               <div className="flex items-center gap-3">
                  <div className="flex items-center bg-background-dark border border-border-dark rounded-xl p-1 shadow-inner">
                     {['Mes actual', '7d', '30d', 'Personalizado'].map(p => (
                        <button
                           key={p}
                           onClick={() => setDateRangeType(p as any)}
                           className={`px-3 py-1.5 text-[10px] font-black uppercase rounded-lg transition-all ${dateRangeType === p ? 'bg-primary text-white shadow-lg' : 'text-text-secondary hover:text-white'}`}
                        >
                           {p}
                        </button>
                     ))}
                  ...
                  <button
                     ...
                     className="flex items-center gap-2 px-5 py-2 bg-primary text-white text-[10px] font-black uppercase rounded-xl hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all active:scale-95"
                  >
            ...
            <div className="flex gap-1">
               {tabs.map(tab => (
                  <button
                     ...
                     className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border-b-2 ${activeTab === tab.id
         ...
         {activeTab === 'agencias' ? (
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-10 pb-40">
                     ...
                     {isLoading ? (
                        <div className="flex items-center justify-center py-20">
                           <div className="text-center space-y-4">
                              ...
                              <p className="text-sm text-text-secondary font-bold">Cargando agencias...</p>
                           </div>
                        </div>
                     ) : error ? (
                        <div className="flex items-center justify-center py-20">
                           <div className="text-center space-y-4">
                              ...
                              <p className="text-xs text-text-secondary">Mostrando datos de ejemplo</p>
                           </div>
                        </div>
                     ) : (
                                 ...
                                 <tr>
                                    <td colSpan={11} className="px-6 py-20 text-center">
                                       <div className="space-y-2">
                                          <Store size={48} className="text-text-secondary/30 mx-auto" />
                                          <p className="text-sm text-text-secondary font-bold">No se encontraron agencias</p>
                                          <p className="text-xs text-text-secondary/60">Intenta ajustar los filtros de búsqueda</p>
                                       </div>
                                    </td>
                                 </tr>
                              ) : (
                                 filteredAgencies.map((a) => {
                                    ...
                                    return (
                                       ...
                                       >
                                          ...
                                          <td className="px-6 py-5 text-center font-bold text-slate-500">{a.rakePercent.toFixed(2)}%</td>
         ...
         {selectedAgency && (
            <div className="fixed inset-0 z-[100] flex justify-end">
               ...
               <div className="relative w-full max-w-xl bg-surface-dark border-l border-border-dark h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">

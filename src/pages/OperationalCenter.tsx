...
const OperationalCenter: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#050505] text-[#e1e1e1] overflow-hidden font-sans selection:bg-[#00ff88]/30">
      ...
      {/* 1. HEADER */}
      <header className="px-6 py-5 bg-[#0d0d0d] border-b border-[#1f1f1f] flex flex-col gap-6 flex-shrink-0 z-30 shadow-2xl">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-[#00ff88]/10 rounded-2xl border border-[#00ff88]/20 text-[#00ff88] shadow-lg shadow-[#00ff88]/5">
              <Zap size={24} className="fill-current" />
            </div>
            <div>
              <h1 className="text-2xl font-black text-white uppercase tracking-tight leading-none italic">Centro Operativo</h1>
              <p className="text-[10px] text-[#666666] font-bold uppercase tracking-[0.3em] mt-1.5 flex items-center gap-2">
                <div className="size-2 rounded-full bg-[#00ff88] animate-pulse"></div> Network Live • Ecuador Command
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
             <button onClick={() => setIsConfigOpen(true)} className={`p-3 bg-[#121212] border rounded-xl hover:text-white transition-all active:scale-95 ${isConfigOpen ? 'border-primary text-primary shadow-lg shadow-primary/10' : 'border-[#1f1f1f] text-slate-400'}`}>
                <Sliders size={20}/>
             </button>
             <button onClick={handleAudit} disabled={isAuditing} className="flex items-center gap-2 px-6 py-3 bg-[#00ff88] text-black text-[11px] font-black uppercase rounded-xl hover:bg-[#00e67a] shadow-xl shadow-[#00ff88]/10 transition-all active:scale-95 disabled:opacity-50">
                {isAuditing ? <RefreshCw size={16} className="animate-spin" /> : <Shield size={16} />} 
                Auditar Red Operativa
              </button>
          </div>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <HeaderMetric label="Capital Total Activos" value={`$${stats.capitalActivos.toLocaleString()}`} sub="Disponible hoy (A)" color="primary" icon={<DollarSign size={18}/>} />
          <HeaderMetric label="Meta Volumen Diario" value={`$${opConfig.targetDailyVolume.toLocaleString()}`} sub="Regla Configurada" color="white" icon={<Target size={18}/>} />
          <HeaderMetric label="Perfiles hoy" value={stats.activeCount} sub="Listos para operar" color="success" icon={<User size={18}/>} />
          <HeaderMetric label="En Descanso" value={stats.restingCount} sub="Flota programada B" color="warning" icon={<PauseCircle size={18}/>} />
        </div>
      </header>

      {/* 2. MAIN SCROLL AREA */}
      <main className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8 pb-40">
        ...
        <section className="bg-[#0d0d0d] border border-[#1f1f1f] rounded-[2.5rem] shadow-xl overflow-hidden mb-20">
          <div className="p-8 border-b border-[#1f1f1f] bg-gradient-to-r from-[#111] to-[#0d0d0d] flex justify-between items-center">
            <div className="flex items-center gap-3">
              <Calendar size={24} className="text-[#00ff88]" />
              <div>
                 <h2 className="text-sm font-black text-white uppercase tracking-widest">Planificador Táctico de Rotación</h2>
                 <p className="text-[10px] text-[#666666] font-bold uppercase mt-1 tracking-tighter">Mes: {currentMonth.toUpperCase()} • CICLO ACTUAL: DÍA {todayIdx + 1}</p>
              </div>
            </div>
            <div className="flex bg-[#050505] p-1.5 rounded-xl border border-[#1f1f1f]">
              <button onClick={() => setPlannerView('Semana')} className={`px-5 py-2 text-[9px] font-black uppercase rounded-lg transition-all ${plannerView === 'Semana' ? 'bg-[#00ff88] text-black shadow-lg shadow-[#00ff88]/20' : 'text-[#666666]'}`}>Semana</button>
              <button onClick={() => setPlannerView('Mes')} className={`px-5 py-2 text-[9px] font-black uppercase rounded-lg transition-all ${plannerView === 'Mes' ? 'bg-[#00ff88] text-black shadow-lg shadow-[#00ff88]/20' : 'text-[#666666]'}`}>Mes Completo</button>
            </div>
          </div>
             ...
             <div className="min-w-[1200px] space-y-6">
                <div className="flex items-center gap-4 px-2">
                   ...
                   <div className="flex-1 flex gap-2">
                      {Array.from({ length: plannerView === 'Semana' ? 7 : daysInMonth }).map((_, i) => {
                        ...
                        return (
                          <div key={i} className={`flex-1 text-center py-4 rounded-2xl border transition-all ${isToday ? 'border-[#00ff88] bg-[#00ff88]/10 shadow-[0_0_20px_rgba(0,255,136,0.15)] ring-1 ring-[#00ff88]/50' : 'border-transparent'}`}>
                            <p className={`text-[10px] font-black uppercase ${isToday ? 'text-[#00ff88]' : 'text-[#444]'}`}>
                              {plannerView === 'Semana' ? ['LUN', 'MAR', 'MIE', 'JUE', 'VIE', 'SAB', 'DOM'][i] : (i+1)}
                            </p>
                            {isToday && (
                               <div className="flex flex-col items-center mt-2">
                                  <div className="size-1.5 bg-[#00ff88] rounded-full animate-pulse shadow-[0_0_8px_rgba(0,255,136,0.8)]"></div>
                                  <span className="text-[7px] font-black text-[#00ff88] mt-1 tracking-widest uppercase">Hoy</span>
                               </div>
                            )}
                          </div>
                        );
                ...
                {profiles.slice(0, 15).map(p => (
                   <div key={p.id} className="flex items-center gap-4 group">
                      <div onClick={() => setSelectedProfileId(p.id)} className="w-60 shrink-0 cursor-pointer p-4 bg-[#050505] border border-transparent hover:border-[#1f1f1f] rounded-2xl transition-all flex items-center justify-between pr-6">
                         <div>
                            <p className="text-[12px] font-black text-white group-hover:text-[#00ff88] truncate">{p.id}</p>
                            <p className="text-[8px] text-[#666666] font-bold uppercase tracking-widest mt-1">{p.owner.split(' ')[0]} • ${p.avgStake} STK</p>
                         </div>
                         {p.agencyId && <Building2 size={12} className="text-primary/30" />}
                      </div>
                      <div className="flex-1 flex gap-2">
                         {Array.from({ length: plannerView === 'Semana' ? 7 : daysInMonth }).map((_, i) => {
                            ...
                            return (
                               <div 
                                  ...
                                  className={`flex-1 h-12 rounded-xl border flex items-center justify-center text-[11px] font-black cursor-pointer transition-all hover:scale-105 active:scale-90 ${
                                    isToday ? 'ring-2 ring-[#00ff88] ring-offset-4 ring-offset-[#0d0d0d]' : ''
                                  } ${
                                    status === 'A' ? 'bg-[#00ff88]/10 border-[#00ff88]/30 text-[#00ff88] shadow-[0_0_10px_rgba(0,255,136,0.05)]' : 'bg-[#121212] border-white/5 text-[#444]'
                                  }`}
                               >
                                  {status}
                               </div>
      ...
      {selectedProfile && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-2xl bg-[#0d0d0d] border-l border-[#1f1f1f] h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-400">
            
            <div className="p-10 border-b border-[#1f1f1f] flex justify-between items-center bg-[#111] relative overflow-hidden">
              <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none"><Monitor size={150} className="text-[#00ff88]"/></div>
              <div className="flex items-center gap-8 relative z-10">
                <div className="size-24 rounded-[2.5rem] bg-gradient-to-br from-[#00ff88]/20 to-[#00ff88]/5 border-2 border-[#00ff88]/30 flex items-center justify-center font-black text-5xl text-[#00ff88] uppercase shadow-2xl">
                  {selectedProfile.id[0]}
                </div>
                <div>
                  <h2 className="text-4xl font-black text-white tracking-tighter leading-none mb-4">{selectedProfile.id}</h2>
                  <div className="flex items-center gap-4">
                     <span className="px-3 py-1 rounded-xl text-[11px] font-black uppercase tracking-widest bg-[#00ff88]/20 text-[#00ff88] border border-[#00ff88]/20">{selectedProfile.bookie}</span>
                     <div className="flex flex-col">
                        <span className="text-[11px] font-black text-[#00ff88] uppercase tracking-[0.2em]">{selectedProfile.owner}</span>
                        {selectedAgency && <span className="text-[9px] text-primary/60 font-bold uppercase flex items-center gap-1"><Building2 size={10}/> {selectedAgency.name}</span>}
                     </div>
                  </div>
                </div>
              </div>
              <button onClick={() => setSelectedProfileId(null)} className="p-4 hover:bg-white/5 rounded-3xl text-[#666666] hover:text-white transition-all"><X size={32}/></button>
            </div>

            <div className="px-10 bg-[#121212] border-b border-[#1f1f1f] flex overflow-x-auto no-scrollbar">
               <DrawerTab active={drawerTab === 'resumen'} onClick={() => setDrawerTab('resumen')} label="Resumen" icon={<LayoutGrid size={16}/>} />
               <DrawerTab active={drawerTab === 'dna'} onClick={() => setDrawerTab('dna')} label="DNA Operativo" icon={<Activity size={16}/>} />
               <DrawerTab active={drawerTab === 'finanzas'} onClick={() => setDrawerTab('finanzas')} label="Finanzas" icon={<Wallet size={16}/>} />
            </div>

            <div className="flex-1 overflow-y-auto p-12 space-y-12 pb-48 custom-scrollbar">
              {drawerTab === 'resumen' && (
                <div className="space-y-12 animate-in fade-in">
                  
                  {selectedAgency && (
                    <div className="p-8 bg-gradient-to-br from-[#00ff88]/10 to-transparent border border-[#00ff88]/20 rounded-[2.5rem] relative overflow-hidden group">
                       <Briefcase size={80} className="absolute -right-4 -bottom-4 text-[#00ff88] opacity-5 group-hover:scale-110 transition-transform"/>
                       <h3 className="text-[10px] font-black uppercase text-[#00ff88] tracking-[0.3em] mb-4">Información de Agencia</h3>
                       <div className="grid grid-cols-2 gap-8">
                          <IdentityItem label="Agencia Asignada" val={selectedAgency.name} icon={<Building2 size={14}/>} />
                          <IdentityItem label="Rake Pactado" val={`${selectedAgency.rake}%`} icon={<TrendingUp size={14}/>} />
                          <IdentityItem label="Responsable" val={selectedAgency.owner} icon={<UserRound size={14}/>} />
                          <IdentityItem label="GGR Casa (Rango)" val={`$${selectedAgency.totalGGR.toLocaleString()}`} icon={<DollarSign size={14}/>} />
                       </div>
                    </div>
                  )}

                  <div className="space-y-5">
                     <h3 className="text-[11px] font-black uppercase text-[#666666] tracking-[0.4em] flex items-center gap-2">
                        <Monitor size={16} className="text-[#00ff88]" /> Lanzadores AdsPower
                     </h3>
                     <div className="grid grid-cols-2 gap-5">
                        <button onClick={() => openOwnerBrowser(selectedProfile.ownerId)} className="p-6 bg-[#121212] border border-[#1f1f1f] rounded-3xl flex flex-col gap-4 group hover:border-[#00ff88]/50 transition-all shadow-xl">
                           <div className="p-3.5 bg-white/5 rounded-2xl border border-white/5 group-hover:text-[#00ff88] transition-colors w-fit"><UserCheck size={28}/></div>
                           <div className="text-left">
                              <p className="text-sm font-black text-white uppercase leading-none mb-1.5">Abrir Perfil Dueño</p>
                              <p className="text-[10px] text-[#666666] font-bold uppercase">{selectedProfile.owner}</p>
                           </div>
                        </button>
                        <button onClick={() => openDirectPageBrowser(selectedProfile.id, selectedProfile.backofficeUrl)} className="p-6 bg-[#00ff88]/10 border border-[#00ff88]/20 rounded-3xl flex flex-col gap-4 group hover:bg-[#00ff88]/20 transition-all shadow-xl">
                           <div className="p-3.5 bg-[#00ff88] text-black rounded-2xl w-fit shadow-xl shadow-[#00ff88]/20"><MousePointer2 size={28}/></div>
                           <div className="text-left">
                              <p className="text-sm font-black text-white uppercase leading-none mb-1.5">Acceso Backoffice Directo</p>
                              <p className="text-[10px] text-[#00ff88] font-bold uppercase">Control de Casa</p>
                           </div>
                        </button>
                     </div>
                  </div>
            ...
            <div className="p-10 border-t border-[#1f1f1f] bg-[#111]/95 flex gap-4 sticky bottom-0 z-30 backdrop-blur-2xl">
               <button onClick={() => setSelectedProfileId(null)} className="flex-1 py-5 text-[11px] font-black uppercase rounded-2xl bg-[#121212] text-white hover:bg-[#1f1f1f] transition-all">Cerrar Terminal</button>
               <button 
                  onClick={() => toggleDayStatus(selectedProfile.id, todayIdx)}
                  className={`flex-[2] py-5 text-[11px] font-black uppercase rounded-2xl transition-all shadow-2xl active:scale-95 ${selectedProfile.schedule[todayIdx] === 'A' ? 'bg-rose-500/10 border border-rose-500/30 text-rose-500 hover:bg-rose-500/20' : 'bg-[#00ff88] text-black hover:bg-[#00e67a]'}`}
               >

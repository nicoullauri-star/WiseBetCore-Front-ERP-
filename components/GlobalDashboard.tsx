...
const GlobalDashboard: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden bg-background-light dark:bg-background-dark font-sans text-slate-900 dark:text-white transition-colors">
      ...
      {/* 1. TOP BAR */}
      <header className="flex-shrink-0 bg-white/80 dark:bg-surface-dark/90 backdrop-blur-md border-b border-gray-200 dark:border-border-dark px-6 py-4 flex justify-between items-center z-20 sticky top-0">
         <div className="flex items-center gap-4">
             <h1 className="text-xl font-black tracking-tight text-slate-900 dark:text-white">DASHBOARD EJECUTIVO</h1>
             ...
             <span className="hidden md:inline text-xs text-text-secondary">Actualizado hace 0 min</span>
         </div>
         
         <div className="flex items-center gap-3">
             {/* DATE FILTER */}
             <div className="flex items-center bg-gray-100 dark:bg-background-dark rounded-lg p-1 border border-gray-200 dark:border-border-dark shadow-inner">
                 {(['MTD', '7D', '30D', '90D', 'YTD'] as const).map(f => (
                     <button 
                        key={f}
                        onClick={() => setDateFilter(f)}
                        className={`px-3 py-1 text-[10px] font-bold uppercase rounded transition-all duration-200 ${dateFilter === f ? 'bg-white dark:bg-border-dark text-slate-900 dark:text-white shadow-sm' : 'text-text-secondary hover:text-slate-900 dark:hover:text-white'}`}
                     >
                         {f === 'MTD' ? 'Mes Actual' : f}
                     </button>
                 ))}
                 <button 
                    ...
                    className={`px-3 py-1 text-[10px] font-bold uppercase rounded transition-all duration-200 ${dateFilter === 'CUSTOM' ? 'bg-white dark:bg-border-dark text-slate-900 dark:text-white shadow-sm' : 'text-text-secondary hover:text-slate-900 dark:hover:text-white'}`}
                 >
                     Personalizado
                 </button>
             </div>
             ...
             <button 
                ...
                className="hidden md:flex items-center gap-2 px-4 py-1.5 bg-primary hover:bg-primary-hover text-white text-xs font-bold rounded-lg transition-all shadow-lg shadow-primary/20 hover:shadow-primary/40 active:scale-95"
             >
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            {/* MAIN CHART */}
            <div className="xl:col-span-2 bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark rounded-xl p-5 shadow-sm flex flex-col min-h-[400px] transition-all duration-300 hover:border-gray-300 dark:hover:border-gray-700">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        <h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">CURVA DE EQUITY GLOBAL</h3>
                        <p className="text-xs text-text-secondary font-medium">Crecimiento de Capital Consolidado + Contexto de Drawdown</p>
                    </div>
